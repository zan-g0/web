...
export const getContactInfoList = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Get total count
    const countResult = await pool.query('SELECT COUNT(*) as total FROM contact_info');
    ...
    const rows = result[0] as ContactInfo[];
...
export const createContactInfo = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const result = await pool.query(
      'INSERT INTO contact_info (company_phone, service_phone, fax, email, postal_code, address, is_active, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())',
      [company_phone, service_phone, fax, email, postal_code, address, is_active || 1]
    );
    ...
    const createdResult = await pool.query(
      'SELECT id, company_phone, service_phone, fax, email, postal_code, address, is_active FROM contact_info WHERE id = ?',
      [newId]
    );

    // createdResult[0] is an array of ContactInfo objects
    const createdRow = createdResult[0] as ContactInfo[];
...
export const updateContactInfo = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const existing = existingResult[0] as ContactInfo[];
    ...
    const updateResult = await pool.query(
      'UPDATE contact_info SET company_phone = ?, service_phone = ?, fax = ?, email = ?, postal_code = ?, address = ?, is_active = ?, updated_at = NOW() WHERE id = ?',
      [company_phone, service_phone, fax, email, postal_code, address, is_active, id]
    );
    ...
    const updatedRow = updatedResult[0] as ContactInfo[];
...
export const deleteContactInfo = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const deleteResult = await pool.query(
      'UPDATE contact_info SET is_active = 0, updated_at = NOW() WHERE id = ?',
      [id]
    );
